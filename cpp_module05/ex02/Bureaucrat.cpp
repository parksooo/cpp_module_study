...
/*   Updated: 2023/09/12 15:55:03 by suhwpark         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::executeForm(const AForm &ob)
{
    try {
        ob.execute(*this);
        std::cout << this->name << " executed " << ob.getName() << std::endl;
    } catch (const std::exception &e) {
        std::cout << this->name << " failed to execute FORM : " << ob.getName() << " reason why " << e.what() << std::endl; 
    }
}
